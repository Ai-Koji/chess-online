...
class Debut extends Component {
    ...
    getFen = (index) => { // возвращает fen элемента из fenList
    ...
    getAlternativeMoveRecursive = (elements, index) => { // рекурсивная функция для getAlternativesMoves
        ...
        for (let i = 0; i < elements.length; i++) {
            if (typeof elements[i] == "object") {
                tempIndex.push(-1)
                elementsFromObject = alternativesMoves.concat(this.getAlternativeMoveRecursive(elements[i], tempIndex))
    ...
    getAlternativesMoves = () => { // получить все возможные ходы для onDrop
        let elements = this.fenList
        this.altenativesMoves = []
        try {
            for (let i = 0; i < this.state.indexNow.length - 1; i++) {
                if (typeof elements[this.state.indexNow[i]] != "object" && i != this.state.indexNow.length - 1) {
                    if (this.fenList.length != 0) {
                        console.log("error in getAlternativesMoves function")
                    }
                    else {
                        console.log("error in getAlternativesMoves function: fenList is empty")
                    }
                    break
                }
            ...
            elements = elements.slice(this.state.indexNow[this.state.indexNow.length - 1] + 1)
            let temp_elements = []
            for (let i = 0; i < elements.length; i++) {
            ...
            this.altenativesMovesIndex = []
            elements = this.getAlternativeMoveRecursive(elements, this.state.indexNow)
            ...
            <div className="container" style={{ display: "block" }}>
                ...
                <div className="grid-container">
                    <section className="board chessboard">
                        <Chessboard position={this.state.fenNow} onPieceDrop={this.onDrop} />
                    </section>
                        ...
                        <div className="game">
                            <table className="main-moves">
                                <tbody>
                                    <tr>
                                        <index>1</index>
                                        <move onClick={() => { this.load([1]) }}>e4</move>
                                        <move>e5</move>
                                    </tr>
                                </tbody>
                            </table>
