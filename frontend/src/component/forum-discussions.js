...
class ForumDiscussions extends React.Component {
    ...
    GetDiscussions = () => {
        ...
        fetch(`http://127.0.0.1:3000/api/forum/discussions/${lastNumber}`)
            ...
            .then((result) => {
                for (let index = 0; index < result.discussions.length; index++) {
                    bodyDiscussions.push(
                        <tr>
                            <td>
                                <a href={`http://127.0.0.1:3001/forum/discussion/${lastNumber}/${result.discussions[index].id}`}>
                                    <h3>
                                        {result.discussions[index].header}
                                    </h3>
                                </a>
                            </td>
